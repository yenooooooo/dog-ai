...
import { Timer, Route, Zap, Flame, Camera, Download } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      ...
      <div className="mx-6 max-h-[90dvh] w-full max-w-sm animate-slide-up overflow-y-auto rounded-mw-xl bg-gradient-to-b from-mw-green-50 to-white px-6 pb-8 pt-6">
        ...
        <button onClick={handleConfirm} className="mt-2 w-full rounded-mw bg-mw-green-500 py-3.5 text-[15px] font-semibold text-white active:scale-[0.97]">
          완료
        </button>
      </div>
    </div>
...
interface SvgData { d: string; startX: number; startY: number }
interface StatsProps {
  svg: SvgData | null; message: string; km: string;
  mins: number; avgSpeed: string; kcal: number; photoCount: number;
}

function WalkCompleteStats({ svg, message, km, mins, avgSpeed, kcal, photoCount }: StatsProps) {
  return (
    <>
      ...
      <div className="mt-4 grid grid-cols-4 gap-2">
        <StatCell icon={<Timer size={14} className="text-mw-green-500" />} label="시간" value={`${mins}분`} />
        <StatCell icon={<Zap size={14} className="text-mw-amber-500" />} label="속도" value={`${avgSpeed}m/분`} />
        <StatCell icon={<Flame size={14} className="text-mw-danger" />} label="칼로리" value={`${kcal}kcal`} />
        <StatCell icon={<Camera size={14} className="text-mw-info" />} label="사진" value={`${photoCount}장`} />
      </div>
    </>
  );
}
