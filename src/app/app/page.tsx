...
export default function AppMainPage() {
  ...
  const {
    routes, selectedIndex, isGenerating, error: routeError,
    generateRoutes, selectRoute, reset,
  } = useRouteStore();
  ...
  const handleMapClick = useCallback((coord: Coordinate) => {
    if (routes.length > 0) return; // 루트 생성 후에는 비활성
  ...
  const handleResetOrigin = () => {
    ...
    toast.success('현재 위치로 돌아왔어요');
  };

  const handleGenerate = useCallback(async () => {
    if (!origin) {
      toast.error('출발 위치를 지정해주세요.');
      return;
    }
    await generateRoutes(origin, duration);
    setSheetExpanded(true);
  }, [origin, duration, generateRoutes]);
  ...
  return (
    <div className="relative h-full overflow-hidden">
      <KakaoMap
        ...
        onMapClick={routes.length === 0 ? handleMapClick : undefined}
      />

      {mapInstance && selectedRoute && (
        <RoutePolyline map={mapInstance} path={selectedRoute.path} />
      )}
      {mapInstance && customOrigin && (
        <OriginMarker map={mapInstance} position={customOrigin} />
      )}
      ...
      {customOrigin && routes.length === 0 && (
        <div className="absolute left-5 top-3 z-30 flex items-center gap-2">
          ...
          <button onClick={handleResetOrigin} className="flex items-center gap-1 rounded-mw bg-white/90 px-3 py-2 shadow-sm backdrop-blur">
        ...
        {routes.length === 0 ? (
          <>
            ...
            <p className="mt-1 text-[13px] text-mw-gray-500">
              {customOrigin ? '📍 지도에서 출발 위치를 지정했어요' : '지도를 탭하면 출발 위치를 변경할 수 있어요'}
            </p>
            <div className="mt-4">
